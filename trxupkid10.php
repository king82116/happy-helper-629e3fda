...
if ($response === false) {
    error_log('TRX Wingo (10min): Failed to contact TronScan API: ' . curl_error($curl));
}
...
if ($response !== false && $httpStatus === 200) {
    ...
    if (json_last_error() === JSON_ERROR_NONE && isset($data['data']) && is_array($data['data'])) {
        ...
        if ($targetBlock === null && isset($data['data'][0])) {
            ...
            error_log('TRX Wingo (10min): No block matched configured second; defaulting to latest block.');
        }
        ...
        error_log('TRX Wingo (10min): Unexpected TronScan API payload.');
    }
} else {
    error_log('TRX Wingo (10min): TronScan API responded with HTTP status ' . ($httpStatus ?? 'unknown') . '.');
}
...
if ($conn) {
    ...
    if ($tathya) {
        ...
        trimLotteryHistoryTable($conn, 'gellaluhogiondu_trx10');
