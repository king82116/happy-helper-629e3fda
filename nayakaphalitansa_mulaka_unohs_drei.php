...
if ($checkRow = mysqli_fetch_assoc($checkResult)) {
	...
	if ($useAPI) {
		...
		curl_setopt_array($ch, [
			CURLOPT_URL => 'https://api.joshclub.fun/wingo_3min.php',
...
if ($samasyesreni['atadaaidi'] != null) {
	...
	if ($gadhipathulidhadi >= 1) {
		$sabutathya = "SELECT
				...
				FROM bajikattuttate_drei WHERE byabaharkarta NOT IN (SELECT balakedara FROM `demo` WHERE `sthiti`='1') AND kalaparichaya = " . $samasyesreni['atadaaidi'];
