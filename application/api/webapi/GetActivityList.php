<?php 
	include "../../conn.php";
	include "../../functions2.php";

	header('Content-Type: application/json; charset=utf-8');
	header('Strict-Transport-Security: max-age=31536000');
	header('Access-Control-Allow-Headers: Origin, X-Requested-With, Content-Type, Accept, Authorization');
	header('Access-Control-Allow-Credentials: true');
	$origin = isset($_SERVER['HTTP_ORIGIN']) ? $_SERVER['HTTP_ORIGIN'] : '';
	header('Access-Control-Allow-Origin: ' . $origin);
	header('vary: Origin');

	date_default_timezone_set("Asia/Kolkata");
	$shnunc = date("Y-m-d H:i:s");

	$res = [
		'code' => 11,
		'msg' => 'Method not allowed',
		'msgCode' => 12,
		'serviceNowTime' => $shnunc,
	];

	$shonubody = file_get_contents("php://input");
	$shonupost = json_decode($shonubody, true);

	if ($_SERVER['REQUEST_METHOD'] != 'GET') {
		if (isset($shonupost['language'], $shonupost['pageNo'], $shonupost['pageSize'], $shonupost['random'], $shonupost['signature'], $shonupost['timestamp'])) {

			$language = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['language']));
			$pageNo = (int)htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['pageNo']));
			$pageSize = (int)htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['pageSize']));
			$random = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['random']));
			$signature = htmlspecialchars(mysqli_real_escape_string($conn, $shonupost['signature']));

			$shonustr = '{"language":'.$language.',"pageNo":'.$pageNo.',"pageSize":'.$pageSize.',"random":"'.$random.'"}';
			$shonusign = strtoupper(md5($shonustr));

			if ($shonusign == $signature) {
				$bearer = explode(" ", $_SERVER['HTTP_AUTHORIZATION']);
				$author = $bearer[1];				
				$is_jwt_valid = is_jwt_valid($author);
				$data_auth = json_decode($is_jwt_valid, true);

				if ($data_auth['status'] === 'Success') {
					$sesquery = "SELECT akshinak FROM shonu_subjects WHERE akshinak = '$author'";
					$sesresult = $conn->query($sesquery);

					if ($sesresult && mysqli_num_rows($sesresult) === 1) {
						$offset = ($pageNo - 1) * $pageSize;
						$bannerSql = "SELECT bannerID, bannerTitle, bannerUrl, jumpType, contents FROM banners ORDER BY bannerID DESC LIMIT $offset, $pageSize";
						$totalSql = "SELECT COUNT(*) as totalCount FROM banners";

						$bannerRes = $conn->query($bannerSql);
						$totalRes = $conn->query($totalSql);
						$totalRow = $totalRes->fetch_assoc();

						$list = [];
						if ($bannerRes->num_rows > 0) {
							while ($row = $bannerRes->fetch_assoc()) {
								$list[] = [
									'bannerID' => (int)$row['bannerID'],
									'bannerTitle' => $row['bannerTitle'],
									'bannerUrl' => $row['bannerUrl'],
									'jumpType' => (int)$row['jumpType'],
									'contents' => $row['contents'],
								];
							}
						}

						$data = [
							'list' => $list,
							'pageNo' => $pageNo,
							'totalPage' => ceil($totalRow['totalCount'] / $pageSize),
							'totalCount' => (int)$totalRow['totalCount'],
						];

						$res['data'] = $data;
						$res['code'] = 0;
						$res['msg'] = 'Succeed';
						$res['msgCode'] = 0;
						http_response_code(200);
						echo json_encode($res);
						exit;
					}
				}
			} else {
				$res['code'] = 5;
				$res['msg'] = 'Wrong signature';
				$res['msgCode'] = 3;
			}
		} else {
			$res['code'] = 7;
			$res['msg'] = 'Param is Invalid';
			$res['msgCode'] = 6;
		}
		http_response_code(200);
		echo json_encode($res);
	} else {
		http_response_code(405);
		echo json_encode($res);
	}
?>
