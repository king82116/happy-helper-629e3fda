		...
		if (isset($shonupost['language']) && isset($shonupost['random']) && isset($shonupost['signature']) && isset($shonupost['timestamp'])) {
			...
			if($shonusign == $signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
						$oedajnahb = getK3IssueTableByTypeId($typeId);
						$intervalMap = [9 => 1, 10 => 3, 11 => 5, 12 => 10];
						$intervalM = $intervalMap[(int) $typeId] ?? 1;
						if ($oedajnahb === null) {
							$res['code'] = 7;
						...
						$samasyesreni = ($samasyephalitansa && $samasyephalitansa->num_rows > 0)
							? mysqli_fetch_array($samasyephalitansa)
							: null;

						if ($samasyesreni === null) {
							$now = date('Y-m-d H:i:s');
							$data = [
								'issueNumber' => '',
								'startTime' => $now,
								'endTime' => date('Y-m-d H:i:s', strtotime('+' . $intervalM . ' minute', strtotime($now))),
								'serviceTime' => $now,
								'intervalM' => $intervalM,
							];
						} else {
