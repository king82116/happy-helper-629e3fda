			...
			if($shonusign != $signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
						...
						$vipRewards = getVipRewardConfig();
						if($taskId == 1){
						    $balanceArray = $vipRewards['levelUp'];
						}
						else if($taskId == 2){
							$balanceArray = $vipRewards['monthly'];
						} else {
						...
						if ($lvl >= 1 && $lvl <= 10) {
							...
							$beforeWagering = computeUserWithdrawalWagering($conn, $shonuid);
							...
							applyBonusNeedToBet($conn, $shonuid, $beforeWagering['amountofCode'], $balance);
