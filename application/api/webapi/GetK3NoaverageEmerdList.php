		...
		if (isset($shonupost['language']) && isset($shonupost['random']) && isset($shonupost['signature']) && isset($shonupost['timestamp']) && isset($shonupost['pageNo']) && isset($shonupost['pageSize']) && isset($shonupost['typeId'])) {
			...
			if($shonusign == $signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
						$oedajnahb = getK3PhalitansaTableByTypeId($typeId);
						if ($oedajnahb === null) {
							$res['code'] = 7;
						...
						trimLotteryHistoryTable($conn, $oedajnahb);
						$pageNo = min(max(1, (int) $pageNo), LOTTERY_HISTORY_MAX_PAGES);
						...
						$pagination = capLotteryHistoryPagination($samasyephalitansa_sankhye, 10, $pageNo);
						
						$data['pageNo'] = (int) $pagination['pageNo'];
