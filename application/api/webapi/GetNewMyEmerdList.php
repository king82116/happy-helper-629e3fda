			...
			if($shonusign == $signature){
				...
				if($data_auth['status'] === 'Success') {
					...
					if($sesnum == 1){
						...
						trimUserBetHistoryForGameType($conn, $gameType, $shonuid);
						...
						if ($samasyephalitansa->num_rows > 0) {
							...
							while ($row = $samasyephalitansa->fetch_assoc()) {
								...
								if($gameType == '1' || $gameType == '13'){
									...
									else if($ergebnis == 1 || $ergebnis == 3 || $ergebnis == 7 || $ergebnis == 9){
										$colour = 'green';
									}
									else if($ergebnis == 2 || $ergebnis == 4 || $ergebnis == 6 || $ergebnis == 8){
										$colour = 'red';
									}
									else{
										$colour = null;
									}
									$data['list'][$i]['colour'] = $colour;
								}
								else{
