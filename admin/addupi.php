...
if(isset($_GET['delete'])){
    ...
    // Check if this is the active UPI
    $check_active = "SELECT sthiti FROM deyya WHERE shonu='$id'";
    $active_result = mysqli_query($conn, $check_active);
    $active_row = mysqli_fetch_assoc($active_result);
    
    if($active_row['sthiti'] == 1) {
        echo '<script type="text/JavaScript"> alert("Cannot delete active UPI ID. Please set another UPI as active first."); </script>';
    } else {
        $delete_query = "DELETE FROM deyya WHERE shonu='$id'";
        if(mysqli_query($conn, $delete_query)){
            echo '<script type="text/JavaScript"> alert("UPI ID Deleted"); </script>';
        } else {
...
if(isset($_POST['edit_upi'])){
    ...
    $update_query = "UPDATE deyya SET maulya='$new_upi' WHERE shonu='$id'";
    if(mysqli_query($conn, $update_query)){
        echo '<script type="text/JavaScript"> alert("UPI ID Updated"); </script>';
    } else {
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-sm-12 mb-4 mb-xl-0">
              <h4 class="font-weight-bold text-dark">UPI Management</h4>
            </div>
          </div> 
          <div class="row">
            <form action="#" id="upiform" method="post" autocomplete="off">
                <div class="d-flex align-items-center">    
                    <input name="newupi" type="text" placeholder="Add an UPI ID" class="flex-grow-1 cool-input" style="height: 40px;" required />
                </div>
          ...
          <div class="row mt-4">
            <?php
                $sel_upi = "SELECT * FROM deyya WHERE sthiti='0' OR sthiti='1' ORDER BY sthiti DESC";
            ...
            ?>
                <form action="#" id="upisave" method="post" autocomplete="off">
                            ...
                            <tr>
                                ...
                                <td class="action-btns">
                                    ...
                                    <a href="?delete=<?php echo $row['shonu']; ?>" class="btn btn-sm btn-danger" 
                                        onclick="return confirm('Are you sure you want to delete this UPI ID?')">
                                        Delete
                                    </a>
                                </td>
                            </tr>
                    ...
                    <button type="submit" class="btn btn-primary cool-button mr-2">Save Active UPI</button>
                </form>
          </div>
