...
if ($demoResult->num_rows > 0) {
    ...
    header('Location: https://jalwagameslive.fun/#/wallet/RechargeHistory');
    exit;
}
...
if (isset($_GET['tyid']) && isset($_GET['amount']) && isset($_GET['uid']) && isset($_GET['sign']) && isset($_GET['urlInfo'])) {
    ...
    $theirurl = $urlarr[0];
    $myurl = 'https://jalwagameslive.fun';
    ...
    if($myurl){
        ...
        // Set up parameters
        $notify_url = "https://jalwagameslive.fun/pay/lgwebhook.php";

        if (!$ramt || !$serial) {
            die("Error: Amount or order ID not provided.");
        }
        $apiUrl= "https://www.lg-pay.com/api/order/create";
        ...
        $params = [
            ...
            'trade_type' => 'INRUPI',
            'order_sn' => $serial,
            'return_url' => 'https://jalwagameslive.fun',
        ];
        ...
        $secretKey = "bX1CcOgnP6cw19j3FcdsrQMpwfTAsAou";
